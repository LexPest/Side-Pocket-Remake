...
using File = DragonOgg.TagLibSharp.File;
...
namespace DragonOgg.MediaPlayer
{
	...
	public class OggFile : IDisposable 
	{
		...
		private File m_TagLibFile;	// TagLibSharp file object
		...
		public OggTagWriteCommandReturn RemoveTag(string TagName)
		{
			...
			XiphComment XC = (XiphComment) m_TagLibFile.GetTag(TagTypes.Xiph, false);
			if (XC != null)
			{
				// Remove the tag
